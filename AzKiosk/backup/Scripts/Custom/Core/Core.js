...
var providerRegion = $('#providersList').val();
...
function hideInputControls(state) {
    ...
    for (var index = 0; index < CT[stateString].IFLDS.length; index++) {
        ...
        var field = CT[stateString].IFLDS[index].split('_')[0];

        //var flowControl = $("#flowDiagram").find('#' + field + 'InputUserControl_controlgroup'),
        var flowControl = $('#flow-controls').find('#' + field + 'InputUserControl_controlgroup'),
            chartControl = $("#chart-controls").find('#' + field + 'InputUserControl_controlgroup'),
            lineChartControl = $("#line-chart-controls").find('#' + field + 'InputUserControl_controlgroup');

        flowControl.show();
        chartControl.show();
        lineChartControl.show();
        ...
        //switch (field) {
        //    case "GWM":
        //        top = +$('#GW_P_node').attr("transform").split(',')[1].split(')')[0];
        //        break;
        //    case "SWM":
        //        top = +$('#SUR_P_node').attr("transform").split(',')[1].split(')')[0];
        //        break;
        //    case "UCON":
        //        top = +$('#UD_P_node').attr("transform").split(',')[1].split(')')[0];
        //        break;
        //    case "ACON":
        //        top = +$('#AD_P_node').attr("transform").split(',')[1].split(')')[0];
        //        break;
        //    case "ICON":
        //        top = +$('#ID_P_node').attr("transform").split(',')[1].split(')')[0];
        //        break;
        //    case "RECM":
        //        top = +$('#REC_P_node').attr("transform").split(',')[1].split(')')[0];
        //        break;
        //    case "DESAL":
        //        top = +$('#SAL_P_node').attr("transform").split(',')[1].split(')')[0] + 10;
        //        break;
        //}

        //flowControl.css("position", "absolute");
        //flowControl.css("top", top + 45);
    }
}
...
function displayIndicators(state) {
    ...
    //params = indicatorParameters['ENVIND'];
    //IndicatorControl(params.divId, params.anIndicatorType, params.ControlId, params.options);

    //params = indicatorParameters['GWSYA'];
    //IndicatorControl(params.divId, params.anIndicatorType, params.ControlId, params.options);

    //params = indicatorParameters['UEF'];
    //IndicatorControl(params.divId, params.anIndicatorType, params.ControlId, params.options);

    //params = indicatorParameters['AGIND'];
    //IndicatorControl(params.divId, params.anIndicatorType, params.ControlId, params.options);

    //params = indicatorParameters['IEF'];
    //IndicatorControl(params.divId, params.anIndicatorType, params.ControlId, params.options);

    //params = indicatorParameters['PEF'];
    //IndicatorControl(params.divId, params.anIndicatorType, params.ControlId, params.options);

    //return console.log('Returning here for testing: displayIndicators');
    ...
    //    }
    //});
    console.log('stateString:', stateString, CT[stateString])
    // QUAY EDIT 3/19/16 begin
    for (var index = 0; index < CT[stateString].INDFLDS.length; index++) {
         ...
         //IndicatorControlsArray[params.anIndicatorType] = new IndicatorControl(params.divId, params.anIndicatorType, params.ControlId, params.options, false, true)
...
(
    function () {
        ...
        //Hide input sliders and show buttons to select values
         console.log('Need to fix the three below me.');
       ...
       // $('.marquee').hide();
        ...
        // STEPTOE EDIT 02/27/18
        // Start fade on CactusJack text
        //speechBubbleFadeOut(7000);
