...
var inputNumber = '<input id="#DID#number" name="#DID#number-#PID#" class="input-number" type="text">';
...
var policyControlCategories = {
    ...
    consumer: ['PCON_P', 'ICON_P', 'UCON_P', 'ACON_P', 'POPGRM_P'],
    climate: ['DSCN_P']
};
...
function buildRadioInputControl(id, fieldName, controlOptions, parentID, hideInputNumber) {
    ...
    html += (hideInputNumber == true ? "" : inputNumber) + divClose;
...
function inputControl2Radio(inputControl, options, mode) {
    ...
    if (mode == undefined || mode == 0) {
        ...
        var htmlFlow = buildRadioInputControl(id, fieldName, inputControlRadioValues[fieldName], 'flowDiagram');
        ...
        parents = ['-flowDiagram', '-barCharts', '-lineCharts'];
...
function updateControlGroup(inputControl, value, noChange) {
    var id = GetRootId($(inputControl).attr('id'));
    console.log('ucp:', $(inputControl).attr('id'));
